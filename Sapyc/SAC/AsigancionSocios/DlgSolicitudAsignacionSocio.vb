Public Class DlgSolicitudAsignacionSocio

    Private ds As New DataSet()
    Private sNameRpt As String = "Selección de socios para asignar propuesta"

    Private dtSocios, dtCISocios, dtPptoSocios, dtSocioEnc, dtCorreosSolicitud, dtAusenciaSocio As New DataTable
    ...
    Private sNombreSocioEnc, sCorreoSocioEnc, sCveOfiEnc, sCveAreaEnc, sAvisoAusencia As String
    Private dIngreso, dMeta, dRecurrente As Decimal

    Public bRefGTI As Boolean
    Public idSac, idPropuesta, idServicio, idIdioma, idMarco, idAusencia As Integer
    ...
    Private Sub LlenarTarjetas()
        ...
        For Each row As DataRow In dtSocios.Rows
            idAusencia = 0
            sAvisoAusencia = ""
            ListarAusenciaSocio(row("CVEEMP").ToString())

            If dtAusenciaSocio.Rows.Count > 0 Then
                sAvisoAusencia = dtAusenciaSocio(0).Item("sAvisoAusencia")
                idAusencia = dtAusenciaSocio(0).Item("idAusencia")
            End If
            ...
            If row("iPuntuacion") = 4 Then
                ...
                AddHandler card.DetalleClick, AddressOf OnSocioDetalleClick
    ...
    Private Sub ListarSocioEncargado(sCveOfi As String, sCveArea As String)
        ...
        End Try
    End Sub
    Private Sub ListarAusenciaSocio(sCveSocio As String)
        ' Aquí iría la lógica para listar los socios, por ejemplo, desde una base de datos
        ' Por ahora, se simula con datos estáticos en el método LlenarTarjetas()
        Try
            Dim sTabla As String = "tbAusenciaSocio"

            LimpiarTabla(dtAusenciaSocio)
            ...
            With ds.Tables
                ...
                dtAusenciaSocio = .Item(sTabla)
            End With
        Catch ex As Exception
            InsertarErrorLog(100, sNameRpt, ex.Message, sCveUsuario, "ListarAusenciaSocio()")
            MsgBox("Hubo un problema al consultar la información en la base de datos, intente de nuevo más tarde.", MsgBoxStyle.Exclamation, My.Settings.NOM_SYS)
            dtAusenciaSocio = Nothing
        End Try
    End Sub
    ...
    Private Sub AsignarSocio()
        ...
        If dtCorreosSolicitud.Rows.Count > 0 Then
            ...
            sCorreoSolicito &= "; " & sCorreoEncargado

            'Incluir al socio encargado para notificarle que ya quedó asignado el socio.
    ...
    Private Sub OnSocioCardClick(tarjetaActual As TarjetaSocio2)
        ...
        If tarjetaActual.Ausencia <> 0 Then
            If MsgBox("El socio seleccionado se encuentra en periodo de ausencia, ¿Desea elegirlo?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, My.Settings.NOM_SYS) = MsgBoxResult.No Then
                Exit Sub
            End If
        End If
    ...
    End Sub
    Private Sub OnSocioDetalleClick(tarjetaActual As TarjetaSocio2)
        Dim dlg As New DlgDetalleAusencia With {
            .idAusencia = tarjetaActual.Ausencia,
            .sCveSocio = tarjetaActual.SocioId
        }
        dlg.ShowDialog()
    End Sub
